				...
				foreach ($fk_tables[$myTable->getName()] as $ref){
					...
					if($is_m2m == false){
						...
						if( $ref['type'] == DBReferenceTypes::REFERENCE_SINGLE_TO_MANY ){
							$content .= parent::TAB().parent::TAB().'Library::requireLibrary(LibraryKeys::APPLICATION_'.str_replace('TBL_','',strtoupper($ref['table'])).'());'.self::LN();
						...
						// m2m
						$primarykey = $pk_col[0];
						$content .= parent::TAB().parent::TAB().'Library::requireLibrary(LibraryKeys::APPLICATION_'.str_replace('TBL_','',strtoupper($ref['ref_table'])).'());'.self::LN();
