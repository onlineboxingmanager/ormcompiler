...
abstract class ORMBuilderBaseClass extends ORMBuilderBase {
	...
	private static final function defaultSelects(DBTable $myTable){
		...
		$content .= parent::TAB().' */'.parent::LN();
		$content .= parent::TAB().'public static function get'.self::makeClassName($myTable->getName()).'List(SQLLimit $myLimit = null){'.parent::LN();
	...
	private static final function buildSingleSelectMethods(DBTable $myTable, DBColumn $myColumn, array $m2m_tables){
		...
		$content .= parent::TAB().' */'.parent::LN();
		$content .= parent::TAB().'public static function get'.self::makeClassName($myTable->getName()).($is_many ? 'List' : '').'By'.parent::formatAttributename($myColumn->getName()).'($'.$col.', SQLLimit $myLimit = null){'.parent::LN();
		if($is_many == false){
	...
	private static final function buildm2mReferences(DBTable $myTable, $fk_tables){
		...
		$content .= parent::TAB().' */'.parent::LN();
		$content .= parent::TAB().'public static function get'.$ziel.'By'.$quelle.'('.$quelle.' $myObject){'.parent::LN();
	...
	private static final function buildReferences(DBTable $myTable, $fk_tables){
		...
		foreach ($fk_tables[$myTable->getName()] as $reference){
			...
			if( $reference['ref_table'] != null OR $reference['type_ziel'] == DBReferenceTypes::REFERENCE_SINGLE_TO_MANY )
				$content .= parent::TAB().'public static function get'.$classname.'ListBy'.$ref_referencename.'('.$ref_classname.' $myObject, SQLLimit $myLimit = null){'.parent::LN();
			else
				$content .= parent::TAB().'public static function get'.$classname.$isList.'By'.$ref_referencename.'('.$ref_classname.' $myObject'.($isList ? ', SQLLimit $myLimit = null' : '').'){'.parent::LN();
			if( $is_manytomany ){
			...
			$content .= parent::TAB().' */'.parent::LN();
			if($is_manytomany OR $is_many)
				$content .= parent::TAB().'public static function get'.$ref_referencename.'ListBy'.$classname.'('.$classname.' $myObject, SQLLimit $myLimit = null){'.parent::LN();
			else
				$content .= parent::TAB().'public static function get'.$ref_referencename.'By'.$classname.'('.$classname.' $myObject){'.parent::LN();
			if( $is_manytomany){
