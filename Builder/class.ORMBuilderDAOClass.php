...
abstract class ORMBuilderDAOClass extends ORMBuilderBase {
	...
	*/
	...
	public static final function build(ORMConfig $myConfig, DBTable $myTable, array $fk_tables, array $m2m_tables){
		...
		$myContent .= parent::TAB().'const TABLENAME = \''.strtolower($myConfig->getDbDatabase()).'.'.strtolower($myTable->getName()).'\';'.parent::LN().parent::LN();
	...
	private static final function buildColumns(DBTable $myTable){
		...
		$content .= parent::TAB().');'.parent::LN();
		$content .= self::LN();
		$content .= parent::TAB().'public static function getColumns(){'.parent::LN();
...
#		}
	...
	private static final function buildFetchFromResult(DBTable $myTable, $fk_tables){
		...
		$content .= parent::TAB().' */'.parent::LN();
		$content .= parent::TAB().'public static final function get'.self::makeClassName($myTable->getName()).'FromResult(ResultSet $myResult, $load_references_from_database = false, $customname = \'\'){'.self::LN();
		...
		$content .= parent::TAB().parent::TAB().'if( empty($row[strtoupper(\''.self::makeClassName($myTable->getName()).'_\'.$pk)]) )'.self::LN();
		...
		*/
		...
		foreach ( $myTable->getColumns() as $col ){
			...
			$content .= parent::TAB().parent::TAB().parent::TAB().'$myObject->set'.self::formatAttributename($col->getName()).'( $myResult->get'.self::formatAttributename($type).'($customname.\''.$colname.'\'));'.self::LN();
	...
	private static final function buildStore(DBTable $myTable){
		...
		$content .= parent::TAB().parent::TAB().parent::TAB().'return self::insert($myObject);'.self::LN();
		$content .= parent::TAB().parent::TAB().' else if( count($myObject->_getModified()) > 0 ) '.self::LN();
		$content .= parent::TAB().parent::TAB().parent::TAB().'return self::update($myObject);'.self::LN();
		$content .= parent::TAB().'}'.self::LN();
		$content .= self::LN();
		return $content;
	}
